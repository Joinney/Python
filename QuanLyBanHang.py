...
class MainApp: 
    def __init__(self,root):
        ...
        self.txt = Text(F3,font = ("Arial",10,"bold"),yscrollcommand = scroll_y.set)
...
# Định nghĩa hàm send_email để mở một cửa sổ mới và nhập email cùng với nội dung
    def send_email(self):
        if  self.txt.get(1.0,END)=='\n':
            messagebox.showerror("lỗi","không có dữ liệu bill")
        else:
            ...
            recipient_frame.grid(row=0, column=0, padx=10, pady=0, sticky='w') 

        # Nhãn và Text cho email của người nhận
            receiver_label = Label(recipient_frame, text="Email to", font=('Arial', 14, 'bold'), bg='green', fg='white')
            receiver_label.grid(row=0, column=0, padx=10, pady=10)

            receiver_entry = Text(recipient_frame, font=('Arial', 12, 'bold'), bd=2, relief=GROOVE, width=30, height=1)
        ...
        # Nhãn cho tin nhắn và Text area cho tin nhắn
            ...
            email_textarea = Text(recipient_frame, font=('Arial', 14, 'bold'), bd=2, relief=GROOVE, width=40, height=20)
            email_textarea.grid(row=2, column=0, columnspan=2, padx=10, pady=10)
            email_textarea.delete(1.0,END)
            email_textarea.insert(END, self.txt.get(1.0, END).replace('\t\t\t', '\t\t\t'))  # Chuyển nội dung từ Text ban đầu sang đây
...
#Function For Text Area
    def welcome_soft(self):
        ...
        self.txt.insert(END,"\t Welcome To Bach Hoa Mini\n")
    ...
    # Lấy nội dung của vùng văn bản
        bill_content = self.txt.get('1.0', 'end-1c')  # 'end-1c' để loại bỏ ký tự cuối cùng (thường là dòng mới)

        # Kiểm tra nội dung có rỗng không
        if not bill_content.strip():
            messagebox.showerror("Lỗi", "Nội dung hóa đơn trống.")
            return
        ...
        try:
            ...
            messagebox.showinfo("THÔNG BÁO", "Đã lưu hóa đơn thành công!")
        except Exception as e:
            messagebox.showerror("Lỗi", f"Có lỗi khi lưu hóa đơn: {str(e)}")
...
#Add Product name , qty and price to bill area
    def bill_area(self):
        ...
        self.txt.insert(END,f"\n")
        self.txt.insert(END,"\tThank you. See you again !\n") 
        # Tính tổng số tiền
